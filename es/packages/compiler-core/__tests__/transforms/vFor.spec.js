...
import { OPEN_BLOCK, CREATE_BLOCK, FRAGMENT, RENDER_LIST, RENDER_SLOT, WITH_DIRECTIVES } from '../../src/runtimeHelpers';
...
describe('compiler: v-for', () => {
    describe('transform', () => {
        test('number expression', () => {
            const { node: forNode } = parseWithForTransform('<span v-for="index in 5" />');
        ...
        test('value', () => {
            const { node: forNode } = parseWithForTransform('<span v-for="(item) in items" />');
            expect(forNode.keyAlias).toBeUndefined();
            expect(forNode.objectIndexAlias).toBeUndefined();
        ...
        test('object de-structured value', () => {
            const { node: forNode } = parseWithForTransform('<span v-for="({ id, value }) in items" />');
            expect(forNode.keyAlias).toBeUndefined();
            expect(forNode.objectIndexAlias).toBeUndefined();
        ...
        test('value and key', () => {
            const { node: forNode } = parseWithForTransform('<span v-for="(item, key) in items" />');
        ...
        test('value, key and index', () => {
            const { node: forNode } = parseWithForTransform('<span v-for="(value, key, index) in items" />');
            expect(forNode.keyAlias).not.toBeUndefined();
            expect(forNode.keyAlias.content).toBe('key');
        ...
        test('skipped key', () => {
            const { node: forNode } = parseWithForTransform('<span v-for="(value,,index) in items" />');
            expect(forNode.keyAlias).toBeUndefined();
            expect(forNode.objectIndexAlias).not.toBeUndefined();
            expect(forNode.objectIndexAlias.content).toBe('index');
            expect(forNode.valueAlias.content).toBe('value');
        ...
        test('skipped value and key', () => {
            const { node: forNode } = parseWithForTransform('<span v-for="(,,index) in items" />');
            expect(forNode.keyAlias).toBeUndefined();
            expect(forNode.objectIndexAlias).not.toBeUndefined();
            expect(forNode.objectIndexAlias.content).toBe('index');
    ...
    describe('errors', () => {
        ...
        test('empty expression', () => {
            ...
            expect(onError).toHaveBeenCalledTimes(1);
            expect(onError).toHaveBeenCalledWith(expect.objectContaining({
                code: 38 /* X_V_FOR_MALFORMED_EXPRESSION */
            }));
        });
    ...
    describe('source location', () => {
        test('value & source', () => {
            ...
            const value = forNode.valueAlias;
            expect(forNode.valueAlias.content).toBe('item');
        ...
        test('bracketed value, key, index', () => {
            ...
            const index = forNode.objectIndexAlias;
    ...
    describe('prefixIdentifiers: true', () => {
        ...
        test('should prefix v-for source w/ complex expression', () => {
            const { node } = parseWithForTransform(`<div v-for="i in list.concat([foo])"/>`, { prefixIdentifiers: true });
            expect(node.source).toMatchObject({
        ...
        test('should not prefix v-for alias', () => {
            const { node } = parseWithForTransform(`<div v-for="i in list">{{ i }}{{ j }}</div>`, { prefixIdentifiers: true });
        ...
        test('should not prefix v-for aliases (multiple)', () => {
            const { node } = parseWithForTransform(`<div v-for="(i, j, k) in list">{{ i + j + k }}{{ l }}</div>`, { prefixIdentifiers: true });
            const div = node.children[0];
            expect(div.children[0].content).toMatchObject({
                type: 8 /* COMPOUND_EXPRESSION */,
                children: [
        ...
        test('should prefix id outside of v-for', () => {
            const { node } = parseWithForTransform(`<div><div v-for="i in list" />{{ i }}</div>`, { prefixIdentifiers: true });
        ...
        test('v-for aliases w/ complex expressions', () => {
            const { node } = parseWithForTransform(`<div v-for="({ foo = bar, baz: [qux = quux] }) in list">
    ...
    describe('codegen', () => {
        function assertSharedCodegen(node, keyed = false, customReturn = false) {
            ...
            const renderListArgs = node.expressions[1]
        ...
        test('basic v-for', () => {
            const { root, node: { codegenNode } } = parseWithForTransform('<span v-for="(item) in items" />');
        ...
        test('value + key + index', () => {
            const { root, node: { codegenNode } } = parseWithForTransform('<span v-for="(item, key, index) in items" />');
            expect(assertSharedCodegen(codegenNode)).toMatchObject({
                source: { content: `items` },
        ...
        test('skipped value', () => {
            const { root, node: { codegenNode } } = parseWithForTransform('<span v-for="(, key, index) in items" />');
            expect(assertSharedCodegen(codegenNode)).toMatchObject({
                source: { content: `items` },
        ...
        test('skipped key', () => {
            const { root, node: { codegenNode } } = parseWithForTransform('<span v-for="(item,,index) in items" />');
            expect(assertSharedCodegen(codegenNode)).toMatchObject({
                source: { content: `items` },
        ...
        test('skipped value & key', () => {
            const { root, node: { codegenNode } } = parseWithForTransform('<span v-for="(,,index) in items" />');
            expect(assertSharedCodegen(codegenNode)).toMatchObject({
                source: { content: `items` },
        ...
        test('template v-for', () => {
            const { root, node: { codegenNode } } = parseWithForTransform('<template v-for="item in items">hello<span/></template>');
            expect(assertSharedCodegen(codegenNode)).toMatchObject({
                source: { content: `items` },
                params: [{ content: `item` }],
                blockArgs: [
        ...
        test('template v-for w/ <slot/>', () => {
            const { root, node: { codegenNode } } = parseWithForTransform('<template v-for="item in items"><slot/></template>');
            expect(assertSharedCodegen(codegenNode, false, true /* custom return */)).toMatchObject({
                source: { content: `items` },
                params: [{ content: `item` }],
                returns: {
                    type: 13 /* JS_CALL_EXPRESSION */,
        ...
        test('v-for on <slot/>', () => {
            const { root, node: { codegenNode } } = parseWithForTransform('<slot v-for="item in items"></slot>');
            expect(assertSharedCodegen(codegenNode, false, true /* custom return */)).toMatchObject({
                source: { content: `items` },
                params: [{ content: `item` }],
                returns: {
        ...
        test('keyed v-for', () => {
            const { root, node: { codegenNode } } = parseWithForTransform('<span v-for="(item) in items" :key="item" />');
        ...
        test('keyed template v-for', () => {
            const { root, node: { codegenNode } } = parseWithForTransform('<template v-for="item in items" :key="item">hello<span/></template>');
            expect(assertSharedCodegen(codegenNode, true)).toMatchObject({
                source: { content: `items` },
                params: [{ content: `item` }],
                blockArgs: [
        ...
        test('v-if + v-for', () => {
            const { root, node: { codegenNode } } = parseWithForTransform(`<div v-if="ok" v-for="i in list"/>`);
        ...
        test('v-for on element with custom directive', () => {
            const { root, node: { codegenNode } } = parseWithForTransform('<div v-for="i in list" v-foo/>');
