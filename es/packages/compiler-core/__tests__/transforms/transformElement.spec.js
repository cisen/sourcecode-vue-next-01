...
import { RESOLVE_COMPONENT, CREATE_VNODE, MERGE_PROPS, RESOLVE_DIRECTIVE, WITH_DIRECTIVES, TO_HANDLERS, helperNameMap, PORTAL, RESOLVE_DYNAMIC_COMPONENT } from '../../src/runtimeHelpers';
import { createObjectProperty } from '../../src/ast';
...
function parseWithElementTransform(template, options = {}) {
    ...
    const codegenNode = ast.children[0].children[0]
        .codegenNode;
    expect(codegenNode.type).toBe(13 /* JS_CALL_EXPRESSION */);
    return {
        root: ast,
        node: codegenNode
    };
...
describe('compiler: element transform', () => {
    ...
    test('v-bind="obj" after static prop', () => {
        const { root, node } = parseWithElementTransform(`<div id="foo" v-bind="obj" />`);
    ...
    test('v-bind="obj" before static prop', () => {
        const { root, node } = parseWithElementTransform(`<div v-bind="obj" id="foo" />`);
        expect(root.helpers).toContain(MERGE_PROPS);
        expect(node.callee).toBe(CREATE_VNODE);
        expect(node.arguments[1]).toMatchObject({
            type: 13 /* JS_CALL_EXPRESSION */,
    ...
    test('v-bind="obj" between static props', () => {
        const { root, node } = parseWithElementTransform(`<div id="foo" v-bind="obj" class="bar" />`);
        expect(root.helpers).toContain(MERGE_PROPS);
        expect(node.callee).toBe(CREATE_VNODE);
        expect(node.arguments[1]).toMatchObject({
            type: 13 /* JS_CALL_EXPRESSION */,
    ...
    test('v-on="obj"', () => {
        const { root, node } = parseWithElementTransform(`<div id="foo" v-on="obj" class="bar" />`);
        expect(root.helpers).toContain(MERGE_PROPS);
        expect(node.callee).toBe(CREATE_VNODE);
        expect(node.arguments[1]).toMatchObject({
            type: 13 /* JS_CALL_EXPRESSION */,
    ...
    test('v-on="obj" + v-bind="obj"', () => {
        const { root, node } = parseWithElementTransform(`<div id="foo" v-on="handlers" v-bind="obj" />`);
        expect(root.helpers).toContain(MERGE_PROPS);
        expect(node.callee).toBe(CREATE_VNODE);
        expect(node.arguments[1]).toMatchObject({
            type: 13 /* JS_CALL_EXPRESSION */,
    ...
    test('should handle <portal> element', () => {
        const { node } = parseWithElementTransform(`<portal target="#foo"><span /></portal>`);
        expect(node.callee).toBe(CREATE_VNODE);
        expect(node.arguments).toMatchObject([
            PORTAL,
            createObjectMatcher({
    ...
    test('directiveTransforms', () => {
        ...
        expect(node.arguments[3]).toMatch(8 /* PROPS */ + '');
        expect(node.arguments[4]).toMatch(`"bar"`);
    });
    test('directiveTransform with needRuntime: true', () => {
        const { root, node } = parseWithElementTransform(`<div v-foo:bar="hello" />`, {
            directiveTransforms: {
                foo() {
                    return {
                        props: [],
        ...
        expect(node.arguments).toMatchObject([
            {
                ...
                arguments: [
                    ...
                    `null`,
                    `null`,
                    genFlagText(32 /* NEED_PATCH */) // should generate appropriate flag
                ]
            },
            {
    ...
    test('directiveTransform with needRuntime: Symbol', () => {
        ...
        expect(node.arguments[1].elements[0].elements[0]).toBe(`_${helperNameMap[CREATE_VNODE]}`);
    });
    test('runtime directives', () => {
        const { root, node } = parseWithElementTransform(`<div v-foo v-bar="x" v-baz:[arg].mod.mad="y" />`);
        expect(root.helpers).toContain(RESOLVE_DIRECTIVE);
        expect(root.directives).toContain(`foo`);
    ...
    test(`props merging: event handlers`, () => {
        const { node } = parseWithElementTransform(`<div @click.foo="a" @click.bar="b" />`, {
            directiveTransforms: {
                on: transformOn
            }
        });
    ...
    test(`props merging: style`, () => {
        const { node } = parseWithElementTransform(`<div style="color: red" :style="{ color: 'red' }" />`, {
    ...
    test(`props merging: class`, () => {
        const { node } = parseWithElementTransform(`<div class="foo" :class="{ bar: isBar }" />`, {
            directiveTransforms: {
                bind: transformBind
            }
        });
    ...
    describe('patchFlag analysis', () => {
        test('TEXT', () => {
            ...
            expect(node2.arguments[3]).toBe(genFlagText(1 /* TEXT */));
            ...
            expect(node3.arguments[3]).toBe(genFlagText(1 /* TEXT */));
        });
        test('CLASS', () => {
            const { node } = parseWithBind(`<div :class="foo" />`);
            expect(node.arguments.length).toBe(4);
            expect(node.arguments[3]).toBe(genFlagText(2 /* CLASS */));
        });
        test('STYLE', () => {
            const { node } = parseWithBind(`<div :style="foo" />`);
            expect(node.arguments.length).toBe(4);
            expect(node.arguments[3]).toBe(genFlagText(4 /* STYLE */));
        });
        test('PROPS', () => {
            const { node } = parseWithBind(`<div id="foo" :foo="bar" :baz="qux" />`);
            expect(node.arguments.length).toBe(5);
            expect(node.arguments[3]).toBe(genFlagText(8 /* PROPS */));
            expect(node.arguments[4]).toBe(`["foo", "baz"]`);
        });
        test('CLASS + STYLE + PROPS', () => {
            const { node } = parseWithBind(`<div id="foo" :class="cls" :style="styl" :foo="bar" :baz="qux"/>`);
            expect(node.arguments.length).toBe(5);
            expect(node.arguments[3]).toBe(genFlagText([2 /* CLASS */, 4 /* STYLE */, 8 /* PROPS */]));
            expect(node.arguments[4]).toBe(`["foo", "baz"]`);
        });
        test('FULL_PROPS (v-bind)', () => {
            const { node } = parseWithBind(`<div v-bind="foo" />`);
            expect(node.arguments.length).toBe(4);
            expect(node.arguments[3]).toBe(genFlagText(16 /* FULL_PROPS */));
        });
        ...
        test('FULL_PROPS (w/ others)', () => {
            const { node } = parseWithBind(`<div id="foo" v-bind="bar" :class="cls" />`);
            expect(node.arguments.length).toBe(4);
            expect(node.arguments[3]).toBe(genFlagText(16 /* FULL_PROPS */));
        });
        test('NEED_PATCH (static ref)', () => {
            ...
            expect(node.arguments.length).toBe(4);
            expect(node.arguments[3]).toBe(genFlagText(32 /* NEED_PATCH */));
        });
        ...
        test('NEED_PATCH (custom directives)', () => {
            ...
            const vnodeCall = node.arguments[0];
            expect(vnodeCall.arguments.length).toBe(4);
            expect(vnodeCall.arguments[3]).toBe(genFlagText(32 /* NEED_PATCH */));
        });
    });
