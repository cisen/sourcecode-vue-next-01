...
describe('compiler: transform v-model', () => {
    ...
    test('should cache update handler w/ cacheHandlers: true', () => {
        ...
        const codegen = root.children[0]
            .codegenNode;
        // should not list cached prop in dynamicProps
        expect(codegen.arguments[4]).toBe(`["modelValue"]`);
        expect(codegen.arguments[1].properties[1].value.type).toBe(20 /* JS_CACHE_EXPRESSION */);
    });
    test('should not cache update handler if it refers v-for scope variables', () => {
        ...
        expect(codegen.arguments[1].properties[1].value.type).not.toBe(20 /* JS_CACHE_EXPRESSION */);
    });
    ...
    describe('errors', () => {
        test('missing expression', () => {
            ...
            expect(onError).toHaveBeenCalledWith(expect.objectContaining({
                code: 47 /* X_V_MODEL_NO_EXPRESSION */
            }));
        });
        test('empty expression', () => {
            ...
            expect(onError).toHaveBeenCalledTimes(1);
            expect(onError).toHaveBeenCalledWith(expect.objectContaining({
                code: 48 /* X_V_MODEL_MALFORMED_EXPRESSION */
            }));
        });
        ...
        test('used on scope variable', () => {
            ...
            expect(onError).toHaveBeenCalledTimes(1);
            expect(onError).toHaveBeenCalledWith(expect.objectContaining({
                code: 49 /* X_V_MODEL_ON_SCOPE_VARIABLE */
            }));
        });
    });
});
