...
describe('compiler: expression transform', () => {
    test('interpolation (root)', () => {
        const node = parseWithExpressionTransform(`{{ foo }}`);
    ...
    test('empty interpolation', () => {
        ...
        const node3 = parseWithExpressionTransform(`<div>{{ }}</div>`);
    ...
    test('interpolation (children)', () => {
        const el = parseWithExpressionTransform(`<div>{{ foo }}</div>`);
    ...
    test('interpolation (complex)', () => {
        const el = parseWithExpressionTransform(`<div>{{ foo + bar(baz.qux) }}</div>`);
        const node = el.children[0];
        expect(node.content).toMatchObject({
    ...
    test('directive value', () => {
        const node = parseWithExpressionTransform(`<div v-foo:arg="baz"/>`);
        const arg = node.props[0].arg;
        expect(arg).toMatchObject({
            type: 4 /* SIMPLE_EXPRESSION */,
            content: `arg`
        });
        const exp = node.props[0].exp;
    ...
    test('should prefix complex expressions', () => {
        const node = parseWithExpressionTransform(`{{ foo(baz + 1, { key: kuz }) }}`);
    ...
    test('should not prefix whitelisted globals', () => {
        const node = parseWithExpressionTransform(`{{ Math.max(1, 2) }}`);
        expect(node.content).toMatchObject({
            type: 8 /* COMPOUND_EXPRESSION */,
    ...
    test('should not prefix reserved literals', () => {
        function assert(exp) {
            const node = parseWithExpressionTransform(`{{ ${exp} }}`);
            expect(node.content).toMatchObject({
                type: 4 /* SIMPLE_EXPRESSION */,
                content: exp
            });
    ...
    test('should not prefix id of a function declaration', () => {
        const node = parseWithExpressionTransform(`{{ function foo() { return bar } }}`);
        expect(node.content).toMatchObject({
            type: 8 /* COMPOUND_EXPRESSION */,
            children: [
                `function `,
    ...
    test('should not prefix params of a function expression', () => {
        const node = parseWithExpressionTransform(`{{ foo => foo + bar }}`);
        expect(node.content).toMatchObject({
            type: 8 /* COMPOUND_EXPRESSION */,
            children: [
                { content: `foo` },
    ...
    test('should prefix default value of a function expression param', () => {
        const node = parseWithExpressionTransform(`{{ (foo = baz) => foo + bar }}`);
        expect(node.content).toMatchObject({
            type: 8 /* COMPOUND_EXPRESSION */,
            children: [
                `(`,
    ...
    test('function params should not affect out of scope identifiers', () => {
        const node = parseWithExpressionTransform(`{{ { a: foo => foo, b: foo } }}`);
        expect(node.content).toMatchObject({
            type: 8 /* COMPOUND_EXPRESSION */,
            children: [
                `{ a: `,
    ...
    test('should prefix default value of function param destructuring', () => {
        const node = parseWithExpressionTransform(`{{ ({ foo = bar }) => foo + bar }}`);
        expect(node.content).toMatchObject({
            type: 8 /* COMPOUND_EXPRESSION */,
            children: [
                `({ `,
    ...
    test('should not prefix an object property key', () => {
        const node = parseWithExpressionTransform(`{{ { foo: bar } }}`);
        expect(node.content).toMatchObject({
            type: 8 /* COMPOUND_EXPRESSION */,
    ...
    test('should not prefix id in a member expression', () => {
        const node = parseWithExpressionTransform(`{{ foo.bar.baz }}`);
        expect(node.content).toMatchObject({
            type: 8 /* COMPOUND_EXPRESSION */,
            children: [
                { content: `_ctx.foo` },
