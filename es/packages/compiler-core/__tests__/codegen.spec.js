import { locStub, generate, createSimpleExpression, createObjectExpression, createObjectProperty, createArrayExpression, createCompoundExpression, createInterpolation, createSequenceExpression, createCallExpression, createConditionalExpression, createCacheExpression } from '../src';
import { CREATE_VNODE, TO_STRING, RESOLVE_DIRECTIVE, helperNameMap, RESOLVE_COMPONENT, CREATE_COMMENT } from '../src/runtimeHelpers';
...
describe('compiler: codegen', () => {
    test('module mode preamble', () => {
        ...
        expect(code).toMatch(`import { ${helperNameMap[CREATE_VNODE]}, ${helperNameMap[RESOLVE_DIRECTIVE]} } from "vue"`);
    ...
    test('function mode preamble', () => {
        ...
        expect(code).toMatch(`const { ${helperNameMap[CREATE_VNODE]}: _${helperNameMap[CREATE_VNODE]}, ${helperNameMap[RESOLVE_DIRECTIVE]}: _${helperNameMap[RESOLVE_DIRECTIVE]} } = _Vue`);
        expect(code).toMatchSnapshot();
    });
    test('function mode preamble w/ prefixIdentifiers: true', () => {
        ...
        expect(code).toMatch(`const { ${helperNameMap[CREATE_VNODE]}, ${helperNameMap[RESOLVE_DIRECTIVE]} } = Vue`);
        expect(code).toMatchSnapshot();
    });
    test('assets', () => {
        const root = createRoot({
        ...
        const root = createRoot({
            hoists: [
                ...
                createObjectExpression([
                    createObjectProperty(createSimpleExpression(`id`, true, locStub), createSimpleExpression(`foo`, true, locStub))
                ], locStub)
            ]
        });
        const { code } = generate(root);
    ...
    test('static text', () => {
        const { code } = generate(createRoot({
            codegenNode: {
                type: 2 /* TEXT */,
                content: 'hello',
                loc: locStub
    ...
    test('Element (callExpression + objectExpression + TemplateChildNode[])', () => {
        const { code } = generate(createRoot({
            codegenNode: createElementWithCodegen([
                ...
                createObjectExpression([
                    createObjectProperty(createSimpleExpression(`id`, true, locStub), createSimpleExpression(`foo`, true, locStub)),
                    createObjectProperty(createSimpleExpression(`prop`, false, locStub), createSimpleExpression(`bar`, false, locStub)),
                ...
                [
                    createElementWithCodegen([
                        ...
                        createObjectExpression([
                            ...
                            createSimpleExpression(`some-key`, true, locStub), createSimpleExpression(`foo`, true, locStub))
                        ], locStub)
                    ])
                ],
                // flag
    ...
    test('ConditionalExpression', () => {
        const { code } = generate(createRoot({
            codegenNode: createConditionalExpression(createSimpleExpression(`ok`, false), createCallExpression(`foo`), createConditionalExpression(createSimpleExpression(`orNot`, false), createCallExpression(`bar`), createCallExpression(`baz`)))
        }));
    ...
    test('CacheExpression', () => {
        const { code } = generate(createRoot({
            cached: 1,
            codegenNode: createCacheExpression(1, createSimpleExpression(`foo`, false))
        }), {
            mode: 'module',
            prefixIdentifiers: true
        });
    ...
    test('CacheExpression w/ isVNode: true', () => {
        const { code } = generate(createRoot({
            cached: 1,
            codegenNode: createCacheExpression(1, createSimpleExpression(`foo`, false), true)
        }), {
            mode: 'module',
            prefixIdentifiers: true
        });
