import { createApp, h, nextTick, createComponent, vModelDynamic, withDirectives } from '@vue/runtime-dom';
const triggerEvent = (type, el) => {
    const event = new Event(type);
    el.dispatchEvent(event);
};
const withVModel = (node, arg, mods) => withDirectives(node, [[vModelDynamic, arg, '', mods]]);
            ...
            render() {
                return [
                    withVModel(h('select', {
                        value: null,
                        'onUpdate:modelValue': setValue.bind(this)
                    }, [h('option', { value: 'foo' }), h('option', { value: 'bar' })]), this.value)
                ];
            }
        });
        app.mount(component, root);
