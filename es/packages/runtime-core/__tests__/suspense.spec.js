import { h, ref, Suspense, render, nodeOps, serializeInner, nextTick, onMounted, watch, onUnmounted, onErrorCaptured } from '@vue/runtime-test';
describe('renderer: suspense', () => {
    ...
    function createAsyncComponent(comp, delay = 0) {
        return {
            setup(props, { slots }) {
    ...
    test('nested suspense (parent resolves first)', async () => {
        ...
        await deps[0];
        await nextTick();
        expect(serializeInner(root)).toBe(`<!----><div>async outer</div><div>fallback inner</div><!---->`);
        expect(calls).toEqual([`outer mounted`]);
        await Promise.all(deps);
        await nextTick();
        expect(serializeInner(root)).toBe(`<!----><div>async outer</div><div>async inner</div><!---->`);
        expect(calls).toEqual([`outer mounted`, `inner mounted`]);
    });
    ...
    it('combined usage (nested async + nested suspense + multiple deps)', async () => {
        ...
        const MiddleComponent = {
            setup() {
                return () => h(AsyncChildWithSuspense, {
                    msg: msg.value
                });
            }
        };
        ...
        await deps[3];
        await nextTick();
        expect(serializeInner(root)).toBe(`<!----><div>nested fallback</div><div>root async</div><!---->`);
        ...
        await Promise.all(deps);
        await nextTick();
        expect(serializeInner(root)).toBe(`<!----><div>nested changed</div><div>root async</div><!---->`);
        expect(calls).toEqual([0, 1, 3, 2]);
        // should update just fine after resolve
        msg.value = 'nested changed again';
        await nextTick();
        expect(serializeInner(root)).toBe(`<!----><div>nested changed again</div><div>root async</div><!---->`);
    });
