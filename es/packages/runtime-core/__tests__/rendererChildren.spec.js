...
import { h, render, nodeOps, serialize, serializeInner, mockWarn } from '@vue/runtime-test';
mockWarn();
...
describe('renderer: keyed children', () => {
    ...
    test('random shuffle', () => {
        ...
        for (let n = 0; n < samples; ++n) {
            ...
            for (let i = 0; i < elms; ++i) {
                expect(serializeInner(elm.children[i])).toBe(i.toString());
                opacities[i] = Math.random()
                    .toFixed(5)
                    .toString();
            }
            const shufArr = shuffle(arr.slice(0));
            render(h('span', arr.map(n => spanNumWithOpacity(shufArr[n], opacities[n]))), root);
            elm = root.children[0];
            for (let i = 0; i < elms; ++i) {
                expect(serializeInner(elm.children[i])).toBe(shufArr[i].toString());
    ...
    test('children with the same key but with different tag', () => {
        ...
        elm = root.children[0];
        expect(elm.children.map(c => c.tag)).toEqual([
            'div',
            'div',
            'div',
            'div'
...
describe('renderer: unkeyed children', () => {
    ...
    test('remove a text node among other elements', () => {
        ...
        elm = root.children[0];
        expect(elm.children.map(c => serialize(c))).toEqual([
            'one',
            '<span>two</span>'
        ]);
