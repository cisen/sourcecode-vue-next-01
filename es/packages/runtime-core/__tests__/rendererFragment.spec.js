import { h, createVNode, render, nodeOps, Fragment, PatchFlags, resetOps, dumpOps, serializeInner } from '@vue/runtime-test';
describe('renderer: fragment', () => {
    ...
    it('patch fragment children (compiler generated, unkeyed)', () => {
        const root = nodeOps.createElement('div');
        render(createVNode(Fragment, null, [h('div', 'one'), 'two'], PatchFlags.UNKEYED_FRAGMENT), root);
        expect(serializeInner(root)).toBe(`<!----><div>one</div>two<!---->`);
        render(createVNode(Fragment, null, [h('div', 'foo'), 'bar', 'baz'], PatchFlags.KEYED_FRAGMENT), root);
        expect(serializeInner(root)).toBe(`<!----><div>foo</div>barbaz<!---->`);
    });
    it('patch fragment children (compiler generated, keyed)', () => {
        const root = nodeOps.createElement('div');
        render(createVNode(Fragment, null, [h('div', { key: 1 }, 'one'), h('div', { key: 2 }, 'two')], PatchFlags.KEYED_FRAGMENT), root);
        expect(serializeInner(root)).toBe(`<!----><div>one</div><div>two</div><!---->`);
        resetOps();
        render(createVNode(Fragment, null, [h('div', { key: 2 }, 'two'), h('div', { key: 1 }, 'one')], PatchFlags.KEYED_FRAGMENT), root);
        expect(serializeInner(root)).toBe(`<!----><div>two</div><div>one</div><!---->`);
        const ops = dumpOps();
        // should be moving nodes instead of re-creating or patching them
        expect(ops).toMatchObject([
    ...
    it('move fragment', () => {
        ...
        expect(serializeInner(root)).toBe(`<div><div>outer</div><!----><div>one</div><div>two</div><!----></div>`);
        resetOps();
        render(h('div', [
            h(Fragment, { key: 2 }, [
                h('div', { key: 2 }, 'two'),
        ...
        expect(serializeInner(root)).toBe(`<div><!----><div>two</div><div>one</div><!----><div>outer</div></div>`);
        const ops = dumpOps();
        // should be moving nodes instead of re-creating them
        expect(ops).toMatchObject([
            // 1. re-order inside the fragment
            { type: "insert" /* INSERT */, targetNode: { type: 'element' } },
            ...
            { type: "insert" /* INSERT */, targetNode: { type: 'comment' } },
            { type: "insert" /* INSERT */, targetNode: { type: 'element' } },
            { type: "insert" /* INSERT */, targetNode: { type: 'element' } },
            { type: "insert" /* INSERT */, targetNode: { type: 'comment' } }
        ]);
    ...
    it('handle nested fragments', () => {
        ...
        expect(serializeInner(root)).toBe(`<!----><div>outer</div><!----><div>one</div><div>two</div><!----><!---->`);
        resetOps();
        render(h(Fragment, [
            h(Fragment, { key: 2 }, [
                h('div', { key: 2 }, 'two'),
        ...
        expect(serializeInner(root)).toBe(`<!----><!----><div>two</div><div>one</div><!----><div>outer</div><!---->`);
        const ops = dumpOps();
        // should be moving nodes instead of re-creating them
        expect(ops).toMatchObject([
            { type: "insert" /* INSERT */, targetNode: { type: 'element' } },
