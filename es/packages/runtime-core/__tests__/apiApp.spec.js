import { createApp, h, nodeOps, serializeInner, mockWarn, provide, inject, resolveComponent, resolveDirective, withDirectives, ref, getCurrentInstance } from '@vue/runtime-test';
describe('api: createApp', () => {
    ...
    test('directive', () => {
        ...
        expect(`Do not use built-in directive ids as custom directive id: bind`).toHaveBeenWarned();
    });
    test('mixin', () => {
        const calls = [];
        const mixinA = {
    ...
    test('config.errorHandler', () => {
        ...
        const count = ref(0);
        const handler = (app.config.errorHandler = jest.fn((err, instance, info) => {
    ...
    test('config.warnHandler', () => {
        const app = createApp();
        let ctx;
        const handler = (app.config.warnHandler = jest.fn((msg, instance, trace) => {
    ...
    describe('config.isNativeTag', () => {
        ...
        test('Component.name', () => {
            ...
            app.mount(Root, nodeOps.createElement('div'));
            expect(`Do not use built-in or reserved HTML elements as component id: div`).toHaveBeenWarned();
        });
