...
`stop,prevent,self,`   +
  ...
  // fn可能是computed对应key的getter函数
  // 给getter添加createReactiveEffect的属性
  ...
  function run(effect, fn, args) {
      ...
      // 如果effect没有存入effectStack，则清空effect的dep
  ...
  // targetMap有多个target-depsMap的对象，depsMap又有多个key-dep的对象，dep又是包含多个effect的Set结构
  // key就是data下面的属性名
  // type就是get/set
  ...
  // 根据effect的类型（同步/nextTick）立即执行还是丢到队列里面
  ...
  */
  ...
  function applyOptions(instance, options, asMixin = false) {
      ...
      if (computedOptions) {
          ...
          for (const key in computedOptions) {
              ...
              if (isFunction(opt)) {
                  // 把封装号的getter存入renderContext
                  // 生成effect并插入instance.effects数组
  ...
  // 将effect存入currentInstance.effects数组
